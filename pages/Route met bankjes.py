...
#@st.cache_resource
def create_map(_gdf, _nodes, _df_route = None, route = False, distance = 0, score = 0, max_gap=0):
	...
	if route and _df_route is not None and len(_df_route) > 0:
		...
		# 🔥 projecteer eerst naar meters
		route_proj = route_gdf.to_crs("EPSG:3857")

		# dissolve route tot 1 lijn
		route_line = route_proj.geometry.unary_union

		# buffer in meters
		route_buffer = route_line.buffer(50)
		...
		bio_proj = bio.to_crs("EPSG:3857")

		# spatial filter
		bankjes_route = bio_proj[bio_proj.intersects(route_buffer)]

		# terug naar WGS84 voor Folium
		bankjes_route = bankjes_route.to_crs("EPSG:4326")

		for point in bankjes_route.geometry:
			folium.Marker(
				[point.y, point.x],
...
def main():
	...
	if add_route:
		...
		route_data, distance, _ = select_best_pareto_route(
			pareto,
			min_dist,
			max_dist,
			max_bankjes_afstand
		)
		distance, score, max_gap = evaluate_route_from_gdf_bankjes(gdf, route_data[6])
		...
		if route_data is None:
			...
			route = True
			df_route = pareto_path_to_gdf(gdf, route_data[6])

	st_folium(create_map(gdf, nodes, df_route, route, distance, score, max_gap), width=1000, height=700, returned_objects=[])
	if pareto_df is not None and len(pareto_df) > 0:

		st.plotly_chart(plot_pareto(pareto_df, selected_gap=max_gap, selected_score=score, selected_distance=distance), use_container_width = True)
