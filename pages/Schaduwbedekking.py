...
#@st.cache_resource
def create_map(_gdf, _nodes, _df_route = None, route = False, distance = 0, score = 0, shadow_pct=0):
...
def main():
	...
	if add_route:
		...
		pareto_labels = generate_pareto_routes_2d(
			gdf,
			start=start,
			end=end,
			L_min=min_dist,
		...
		if len(pareto_df) == 0:
			...
			# 1. FILTER op minimale schaduw
			feasible = pareto_df[
				pareto_df["shadow_density"] >= (min_shadow / 100)
			]
			feasible_rows = feasible.to_dict("records")
			if len(feasible) == 0:
				st.error("Geen routes voldoen aan minimale schaduwbedekking")
			else:

				# 2. kies beste score binnen constraint
				best_route = max(
					feasible_rows,
				...
				distance = best_route["afstand"]
				score = best_route["score_density"]
				shadow_pct = best_route["shadow_density"] * 100

				df_route = pareto_path_to_gdf(gdf, path)
				route = True

	st_folium(create_map(gdf, nodes, df_route, route, distance, score, shadow_pct), width=1000, height=700, returned_objects=[])
	if pareto_df is not None and len(pareto_df) > 0:

		st.plotly_chart(plot_pareto(pareto_df, selected_shadow=shadow_pct, selected_score=score, selected_distance=distance), use_container_width = True)
